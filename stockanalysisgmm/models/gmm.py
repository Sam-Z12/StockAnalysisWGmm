...
class GmmCandleChart(CandleChart):
    ...
    def model_generator(self, start_index=214, end_index=None, period=SIM_PERIOD,):
        ...
        if start_index < period:
            raise ValueError("Start index must be equal to or greater than the period")
        if end_index == None:
            end_index = (len(self.candles)-1)
        if start_index <= end_index:
            train_set_start_index = start_index - SIM_PERIOD
            training_set = create_training_set(x_train_dict=self.indicators, start_index=train_set_start_index, end_index=start_index)
    ...
    def init_model(self,start_index=None, end_index=None):
        ...
        self.training_set = create_training_set(x_train_dict=self.indicators, start_index=start_index)
    ...
    def create_label_groups(self):
        ...
        for key in list(lab_dict.keys()):
            label = lab_dict[key]
            candle_group = [candle for candle in self.candles if candle.label == label] 
            groups.append(LabelGroup(key=key, label=label, list_candles=candle_group))
        self.groups = groups
        return self.groups
    ...
    def plot(self):
        ...
        candlestick_ohlc(ax, candles_df.values, width=0.5, colorup='green', colordown='red', alpha=1)
...
class GmmSimulation:
    ...
    def sim_label_switchs(self, candle):
        ...
        if "sim_label_switchs" not in self.sim_storage:
            ...
            if label != prev_label:
                self.sim_storage['sim_label_switchs']['switchs']['switch_to'].append(label)
                self.sim_storage['sim_label_switchs']['switchs']['switch_date'].append(candle.date())
